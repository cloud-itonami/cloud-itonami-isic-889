(ns socialworkops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (SocialWorkAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SocialWorkGovernor (:govern)
  ...
  human operator/social worker. The approver resumes with
...
(defn build
  ...
  `socialworkops.store/Store`).
  opts:
    :advisor      -- a `socialworkops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SocialWorkAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SocialWorkGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :client-id (:client-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :client-id (:client-id request) :by (:by approval)}]}
            {:disposition :hold
